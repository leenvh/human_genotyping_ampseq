...
parser.add_argument('--demux-script', type=str, required=True, help='Path to demux_nanopore_amplicon.py')
...
for bc in plate_layout:
    ...
    # Run demultiplexing
    sp.run(f"{args.demux_script} --max-mismatch {args.max_mismatch} --fastq {fastq_file} --barcodes {tmp_barcode_file} --edge-size 100 --log-prefix {bc}", shell=True)
